...
모듈형 주식 분석 프로그램 (Font Warning 수정, 4개 정보 패널, 4가지 폭락 전략 분석)

Author: AI Assistant & User
Version: 2.1.0 - Font Warning 수정 및 안정성 개선
"""
...
class BosPlanApp:
    """1Bo's Plan 메인 애플리케이션 v2.1 - Font Warning 수정"""
    ...
    def create_analysis_tab(self):
        ...
        try:
            ...
            tk.Label(left_panel, text=f"{ICONS['fire']} 인기 종목:", font=('Segoe UI', 11, 'bold')).pack(anchor=tk.W, pady=(10, 5))
            ...
            tk.Label(left_panel, text=f"{ICONS['folder']} 내 종목:", font=('Segoe UI', 11, 'bold')).pack(anchor=tk.W, pady=(10, 5))
...
{ICONS['success']} 새 기능: 자동 포트폴리오 설정
• 평단가 자동 설정 (현재가의 90%)
• 보유량 자동 설정 (1000주)
• 한국/미국 주식별 예산 조정

{ICONS['chart']} UI 개선: 4개 정보 패널
• 종목 정보 / 포지션 / 기술적 분석 / 매매 신호
• 차트 공간 확보로 더 나은 시각화

{ICONS['warning']} Font Warning 수정 (v2.1)
• 이모지 폰트 경고 메시지 완전 제거
• 안정적인 텍스트 아이콘 사용
• 더 나은 호환성과 안정성
...
시작하려면 좌측 설정을 입력하고 Calculate를 누르세요!""")
    ...
    def create_crash_tab(self):
        ...
        try:
            ...
            info_frame = ttk.LabelFrame(crash_frame, text=f"{ICONS['warning']} 폭락장 대응 전략", padding="15")
            ...
            tk.Button(control_panel, text=f"{ICONS['warning']} 최적 손절가 계산", command=self.calculate_cutloss).pack(fill=tk.X, pady=5)
            tk.Button(control_panel, text=f"{ICONS['copy']} AI 자문 리포트", command=self.generate_ai_report).pack(fill=tk.X, pady=5)
...
{ICONS['info']} 핵심 질문: "지금 손절해야 할까? 아니면 분할매수를 계속해야 할까?"
...
{ICONS['warning']} 특히 레버리지 ETF (SOXL, TQQQ 등)는 특별 관리가 필요합니다!
...
시작하려면 왼쪽 분석 도구를 사용하세요.""")
    ...
    def download_and_auto_setup(self):
        ...
        try:
            ...
            if not symbol:
                messagebox.showwarning(f"{ICONS['warning']}", "종목 코드를 입력해주세요.")
                return
            ...
            if result and result['success']:
                ...
                success_msg = f"""{ICONS['success']} 자동 설정 완료!

{ICONS['chart']} 종목: {company_name} ({symbol})
{ICONS['investment']} 현재가: {price_text}
{ICONS['chart']} 평단가: {avg_price_text} (현재가의 90%)
{ICONS['folder']} 보유량: {auto_position:,}주
...
Crash Strategy 탭에서 위험 분석을 하세요!"""
                ...
                messagebox.showinfo(f"{ICONS['chart']} 자동 설정 완료", success_msg)
                
            else:
                messagebox.showerror(f"{ICONS['error']}", result['message'] if result else "다운로드 실패")
    ...
    def analyze_stock(self):
        ...
        try:
            ...
            if data is None:
                messagebox.showwarning(f"{ICONS['warning']}", "먼저 데이터를 다운로드해주세요.")
                return
            ...
            if analysis_result:
                ...
                messagebox.showinfo(f"{ICONS['success']} 분석 완료", success_msg)
            else:
                messagebox.showerror(f"{ICONS['error']}", "분석 실패")
    ...
    def calculate_investment(self):
        ...
        try:
            try:
                ...
                messagebox.showerror(f"{ICONS['error']}", "올바른 숫자를 입력해주세요.")
                return
            ...
            if current_price is None:
                ...
                if data is not None:
                    ...
                    messagebox.showwarning(f"{ICONS['warning']}", "현재가를 입력하거나 종목 데이터를 로드해주세요.")
                    return
            ...
            if result:
                ...
                messagebox.showerror(f"{ICONS['error']}", "계산 실패")
    ...
    def use_current_price(self):
        ...
        try:
            ...
            if data is not None:
                ...
                if DataValidator.is_korean_stock(symbol):
                    self.current_price_var.set(f"{current_price:.0f}")
                    messagebox.showinfo(f"{ICONS['success']}", f"현재가 ₩{current_price:,.0f}이 입력되었습니다.")
                else:
                    self.current_price_var.set(f"{current_price:.2f}")
                    messagebox.showinfo(f"{ICONS['success']}", f"현재가 ${current_price:.2f}이 입력되었습니다.")
            else:
                messagebox.showwarning(f"{ICONS['warning']}", "먼저 종목 데이터를 로드해주세요.")
        except Exception as e:
    ...
    def assess_risk(self):
        ...
        try:
            ...
            if risk_result:
                ...
                risk_text = f"""{ICONS['chart']} 위험 평가 결과
...
권장사항: {risk_result['recommendation']}"""
                ...
                messagebox.showinfo(f"{ICONS['chart']} 위험 평가", risk_text)
            else:
                messagebox.showerror(f"{ICONS['error']}", "위험 평가 실패")
    ...
    def analyze_crash_situation(self):
        ...
        try:
            ...
            if crash_result:
                ...
                result_text = f"""{ICONS['crash']} {APP_NAME} v{APP_VERSION} 폭락장 분석 결과
                    ...
                    try:
                        ...
                        if filename:
                            with open(filename, 'w', encoding='utf-8') as f:
                                f.write(report)
                            messagebox.showinfo(f"{ICONS['success']}", f"리포트가 저장되었습니다:\n{filename}")
                    except Exception as e:
                        messagebox.showerror(f"{ICONS['error']}", f"저장 실패: {e}")
                
                # 버튼들
                tk.Button(button_frame, text=f"{ICONS['copy']} 클립보드에 복사", 
                         command=copy_report, font=('Segoe UI', 11, 'bold'),
                         bg='#4CAF50', fg='white').pack(side=tk.LEFT, padx=(0, 10))
                
                tk.Button(button_frame, text=f"{ICONS['save']} 파일로 저장", 
