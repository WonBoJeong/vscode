...
모듈형 주식 분석 프로그램 (기술적 분석 강화)

Author: AI Assistant & User
Version: 1.1.0 - 매매 시점 분석 기능 추가
"""
...
class BosPlanApp:
    ...
    def create_analysis_tab(self):
        ...
        try:
            ...
            # Stock Information 패널
            info_panel = ttk.LabelFrame(right_panel, text="📋 Stock Information", padding="10")
            info_panel.pack(fill=tk.X, pady=(0, 10))
            
            self.info_label = tk.Label(info_panel, text="주식을 선택하고 분석해주세요.", 
                                     wraplength=800, justify=tk.LEFT)
            self.info_label.pack(anchor=tk.W)
            
            # 차트 패널 - 🎯 여기에 차트 컨트롤 패널 추가
...
🎉 복원된 기능: 차트 컨트롤
• 기간 선택: 30일, 90일, 1년, 3년, 10년
• 이동평균선: MA5, MA20, MA60, MA200 체크박스
• 차트 저장 및 정보 표시 기능
...
"""
                ...
                if crash_result['is_leverage_etf']:
                    ...
                    result_text += "• 30일 이상 보유 절대 금지\n"
                
                result_text += f"\n⚠️ 분석 시간: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
                
                self.crash_results.delete('1.0', tk.END)
                self.crash_results.insert('1.0', result_text)
            else:
                messagebox.showerror("❌", "폭락 분석 실패")
    ...
    def calculate_cutloss(self):
        ...
        try:
            ...
            if cutloss_result:
                result_text = f"""✂️ {APP_NAME} 최적 손절가 계산
...
"""
                ...
                if cutloss_result['is_leverage']:
                    result_text += "\n⚡ 레버리지 ETF 특별 주의:\n"
                    result_text += "• 일반 주식보다 엄격한 손절 기준 적용\n"
                    result_text += "• 감정에 휘둘리지 말고 기계적 실행\n"
            ...
            self.error_handler.handle_exception(e, True, "Cutloss calculation")
    ...
    def generate_ai_report(self):
        ...
        try:
            ...
            if crash_result:
                report = self.crash_analyzer.generate_situation_report(crash_result)
                ...
                tk.Label(main_frame, text="📋 AI 투자 자문용 상황 리포트", 
                        font=('Segoe UI', 16, 'bold')).pack(pady=(0, 15))
                
                tk.Label(main_frame, text="아래 리포트를 복사하여 AI에게 전문 투자 자문을 요청하세요.", 
                ...
                tk.Button(button_frame, text="📋 클립보드에 복사", 
                         command=copy_report).pack(side=tk.LEFT)
    ...
    def update_stock_info(self, data, symbol, analysis=None):
        """주식 정보 업데이트 - 🎯 평단가 용어 사용 및 기술적 분석 추가"""
        try:
            ...
            if is_korean:
                ...
                info_text += f"📈 일간변동: ₩{change:+,.0f} ({change_pct:+.2f}%)\n"
            else:
                info_text += f"💰 현재가: ${latest_price:.2f}\n"
                info_text += f"📈 일간변동: ${change:+.2f} ({change_pct:+.2f}%)\n"
            
            info_text += f"📅 데이터: {len(data)}일간\n"
            ...
            try:
                ...
                if avg_price and position > 0:
                    ...
                    if is_korean:
                        info_text += f"  평단가: ₩{avg_price:,.0f}\n"
                        info_text += f"  보유량: {position:,.0f}주\n"
                        info_text += f"  평가손익: ₩{pnl:+,.0f} ({pnl_pct:+.2f}%)\n"
                    else:
                        info_text += f"  평단가: ${avg_price:.2f}\n"
                        info_text += f"  보유량: {position:,.0f}주\n"
                        info_text += f"  평가손익: ${pnl:+,.2f} ({pnl_pct:+.2f}%)\n"
            except ValueError:
                pass
            ...
            if analysis:
                info_text += self._format_technical_analysis(analysis, is_korean)
    ...
    def _format_technical_analysis(self, analysis, is_korean):
        """기술적 분석 결과 포맷팅"""
        try:
            text = "\n🔍 기술적 분석:\n"
            ...
            if 'recent_stats' in analysis and analysis['recent_stats']:
                ...
                if is_korean:
                    text += f"  3일 평균: ₩{avg_3_days:,.0f} ({diff_pct:+.1f}%)\n"
                else:
                    text += f"  3일 평균: ${avg_3_days:.2f} ({diff_pct:+.1f}%)\n"
                
                # 신호 표시
                if stats['trend_signal'] == 'ABOVE_AVG':
                    text += f"  📈 평균 상회 (강세)\n"
                elif stats['trend_signal'] == 'BELOW_AVG':
                    text += f"  📉 평균 하회 (약세)\n"
                else:
                    text += f"  ➡️ 평균 근처 (보합)\n"
            ...
            if 'confidence_interval' in analysis and analysis['confidence_interval']:
                ...
                if is_korean:
                    text += f"  신뢰구간: ₩{ci['lower_bound']:,.0f} ~ ₩{ci['upper_bound']:,.0f}\n"
                else:
                    text += f"  신뢰구간: ${ci['lower_bound']:.2f} ~ ${ci['upper_bound']:.2f}\n"
                
                # 포지션 신호
                if ci['signal'] == 'POTENTIAL_BUY':
                    text += f"  💚 신뢰구간 하단 (매수 고려)\n"
                elif ci['signal'] == 'POTENTIAL_SELL':
                    text += f"  🔴 신뢰구간 상단 (매도 고려)\n"
                else:
                    text += f"  🟡 신뢰구간 내 (관망)\n"
            ...
            if 'sp500_comparison' in analysis and analysis['sp500_comparison']:
                sp500 = analysis['sp500_comparison']
                stock_return = sp500['stock_return']
                sp500_return = sp500['sp500_return']
                relative_perf = sp500['relative_performance']
                
                text += f"  SP500 vs 종목: {sp500_return:+.1f}% vs {stock_return:+.1f}%\n"
                
                if sp500['outperforming']:
                    text += f"  🎯 SP500 대비 +{relative_perf:.1f}% 우수\n"
                else:
                    text += f"  📊 SP500 대비 {relative_perf:.1f}% 부진\n"
                
                if 'beta' in sp500:
                    text += f"  베타: {sp500['beta']:.2f} (상관도: {sp500['correlation']:.2f})\n"
            
            # 매매 결정 정보
            if 'trading_decision' in analysis and analysis['trading_decision']:
                decision = analysis['trading_decision']
                
                text += f"\n🎯 매매 신호: {self._translate_decision(decision['decision'])}\n"
                text += f"  신뢰도: {self._translate_confidence(decision['confidence'])}\n"
                text += f"  근거: {decision['reasoning']}\n"
            ...
            self.logger.error(f"Technical analysis formatting failed: {e}")
            return "\n🔍 기술적 분석: 분석 불가\n"
